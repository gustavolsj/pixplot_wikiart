...
Data.prototype.onTextureProgress = function (texIdx, progress) {
  this.textureProgress[texIdx] =
    progress / this.textures[texIdx].getAtlasCount(texIdx);
...
Layout.prototype.set = function (layout, enableDelay) {
  ...
  setTimeout(
    function () {
      get(
        getPath(data.layouts[layout][layoutType]),
        function (pos) {
...
Selection.prototype.addModalEventListeners = function () {
  ...
  this.elems.modalButton.addEventListener(
    "click",
    function (e) {
      this.elems.modalTarget.innerHTML = _.template(
        this.elems.modalTemplate.textContent,
      )({
...
Selection.prototype.getSelectedImages = function () {
  return data.json.images.filter(
    function (i, idx) {
...
Selection.prototype.insideBox = function (i) {
  ...
  if (!box) return false;
  return (
    i.x >= box.x.min && i.x <= box.x.max && i.y >= box.y.min && i.y <= box.y.max
  );
};
...
Selection.prototype.update = function () {
  ...
  // find the cumulative length of the line up to each point
  var geometry = new THREE.BufferGeometry().setFromPoints(points);
  var lengths = new THREE.BufferAttribute(
    new Float32Array(this.getLineLengths(points)),
    1,
  );
...
function Filter(obj) {
  ...
  select.onchange = function (e) {
    ...
    if (self.selected == "All Values") {
      ...
      var filename =
          self.selected.replace(/\//g, "-").replace(/ /g, "__") + ".json",
